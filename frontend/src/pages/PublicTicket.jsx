        ...
        return (
            <div className="min-h-screen flex flex-col items-center justify-center p-4">
                <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center animate-fade-in">
                    ...
                    <h2 className="text-2xl font-bold text-gray-900 mb-2">¡Ticket Enviado!</h2>
                    <p className="text-gray-600 mb-6 font-medium">
                        Hemos recibido tu solicitud. Guarda los siguientes datos para seguir el estado de tu ticket y chatear con soporte:
                    </p>

                    <div className="bg-gray-50 rounded-xl p-6 mb-8 border border-gray-100 space-y-4">
                        <div>
                            <p className="text-xs text-gray-400 uppercase font-bold tracking-wider mb-1">ID del Ticket</p>
                            <p className="text-sm font-mono text-gray-700 break-all">{accessInfo.id}</p>
                        </div>
                        <div>
                            <p className="text-xs text-gray-400 uppercase font-bold tracking-wider mb-1">Código de Acceso (Pin)</p>
                            <p className="text-2xl font-black text-blue-600 tracking-widest">{accessInfo.code}</p>
                        </div>
                    </div>
    ...
    return (
        <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden animate-fade-in">
                <div className="bg-blue-600 p-8 text-white">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h1 className="text-2xl font-bold italic tracking-tight">MuniSupport Chiquinquirá</h1>
                    </div>
                    <p className="text-blue-100">Portal Público de Soporte Técnico</p>
                </div>
                ...
                <div className="p-8">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                                <Phone className="w-4 h-4 text-gray-400" /> Teléfono (Opcional)
                            </label>
                            <input
                                type="text"
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-semibold text-gray-700">Título de la incidencia *</label>
                            <input
                                type="text"
                                name="titulo"
                                required
                        ...
                        <div className="space-y-4 col-span-1 md:col-span-2">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                                <div className="space-y-2">
                                    <label className="text-sm font-semibold text-gray-700">Código de Oficina (PIN de 5 dígitos) *</label>
                                    ...
                                    {verifiedOffice ? (
                                        <div className="flex items-center gap-2 p-3 bg-green-50 text-green-700 rounded-xl border border-green-100 flex-1 animate-pulse-subtle">
                                            ...
                                            <div className="text-xs">
                                                <p className="font-bold">Oficina Identificada:</p>
                                                <p>{verifiedOffice.dependencia} {verifiedOffice.seccion ? `- ${verifiedOffice.seccion}` : ''}</p>
                                            </div>
                                        </div>
                                    ) : pinError ? (
                                        <div className="flex items-center gap-2 p-3 bg-red-50 text-red-700 rounded-xl border border-red-100 flex-1">
                                            <AlertCircle className="w-5 h-5" />
                                            <span className="text-xs font-semibold">{pinError}</span>
                                        </div>
                                    ) : (
                                        <div className="flex items-center gap-2 p-3 bg-gray-50 text-gray-500 rounded-xl border border-gray-100 flex-1">
                                            <LifeBuoy className="w-5 h-5" />
                                            <span className="text-xs italic">Ingrese el código para validar la oficina</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="relative group">
                                ...
                                >
                                    ...
                                    <span className="text-sm text-gray-500 group-hover:text-blue-600 truncate">
                                        {file ? file.name : 'Subir imagen o doc'}
                                    </span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div className="pt-4 flex gap-4">
                            ...
                            >
                                ...
                                {loading ? 'Enviando...' : 'Enviar Reporte de Oficina'}
                            </button>
                        </div>
                    </form>
                </div>
            ...
            <p className="text-center text-gray-400 text-sm mt-8">
                &copy; {new Date().getFullYear()} MuniSupport Chiquinquirá. Todos los derechos reservados.
            </p>
        </div>
    );
};
